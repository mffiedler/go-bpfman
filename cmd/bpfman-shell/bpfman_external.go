...
// replDispatchExternal forks the bpfman binary with the
// command's textual args, appends -o json if absent, captures
// stdout, and decodes via decodeBpfmanResult.
func replDispatchExternal(ctx context.Context, args []shell.Arg) (shell.Value, error) {
	...
	for i, a := range args[1:] {
		...
		if err != nil {
			return shell.Value{}, fmt.Errorf("bpfman arg %d: %w", i+1, err)
		}
	...
	return decodeBpfmanResult(args[1:], stdout.Bytes())
}

// argToCLIText renders a single shell.Arg into the textual form
...
func argToCLIText(a shell.Arg) (string, error) {
	switch v := a.(type) {
	case shell.WordArg:
		return v.Text, nil
...
//
// The dispatch table mirrors inferBpfmanBindShape in
// bindshapes.go: any (noun, verb) that yields a typed Shape
// there must yield a Value with that OriginKind here. Tests
// that run the same script under both modes are the parity
// check.
func decodeBpfmanResult(args []shell.Arg, stdout []byte) (shell.Value, error) {
	if len(args) < 2 {
		return shell.Value{}, nil
	}
	noun := repl.ArgText(args[0])
	verb := repl.ArgText(args[1])
	switch noun {
	case "program":
		switch verb {
		case "get":
			...
			if err := json.Unmarshal(stdout, &p); err != nil {
				return shell.Value{}, fmt.Errorf("decode Program: %w", err)
			}
			v, err := shell.ValueFromStruct(p)
			if err != nil {
				return shell.Value{}, err
			}
			return v.WithKind(shell.OriginProgram), nil
		case "list":
			var result bpfman.ProgramListResult
			if err := json.Unmarshal(stdout, &result); err != nil {
				return shell.Value{}, fmt.Errorf("decode ProgramListResult: %w", err)
			}
			return shell.ValueFromStruct(result)
		case "load":
			var result bpfman.LoadResult
			if err := json.Unmarshal(stdout, &result); err != nil {
				return shell.Value{}, fmt.Errorf("decode LoadResult: %w", err)
			}
			return shell.ValueFromStruct(result)
		case "unload", "delete":
			return shell.Value{}, nil
	...
	case "link":
		...
		case "attach", "get":
			...
			if err := json.Unmarshal(stdout, &l); err != nil {
				return shell.Value{}, fmt.Errorf("decode Link: %w", err)
			}
			v, err := shell.ValueFromStruct(l)
			if err != nil {
				return shell.Value{}, err
			}
			return v.WithKind(shell.OriginLink), nil
		case "list":
			var result bpfman.LinkListResult
			if err := json.Unmarshal(stdout, &result); err != nil {
				return shell.Value{}, fmt.Errorf("decode LinkListResult: %w", err)
			}
			return shell.ValueFromStruct(result)
		case "detach", "delete":
			return shell.Value{}, nil
