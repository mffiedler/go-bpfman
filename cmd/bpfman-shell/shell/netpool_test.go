...
func TestAcquirePoolSlot_AssignsSlotOneFirst(t *testing.T) {
	...
	lease, err := AcquirePoolSlot(context.Background(), poolReq(root, "test.bpfman:1", "ns0", "vea0"))
	require.NoError(t, err)
...
func TestAcquirePoolSlot_DistinctSlotsWhileHeld(t *testing.T) {
	...
	first, err := AcquirePoolSlot(context.Background(), poolReq(root, "a", "ns-a", "vea-a"))
	require.NoError(t, err)
	defer ReleasePoolSlot(first, "ns-a", "vea-a")

	second, err := AcquirePoolSlot(context.Background(), poolReq(root, "b", "ns-b", "vea-b"))
	require.NoError(t, err)
...
func TestAcquirePoolSlot_FIFOOnRelease(t *testing.T) {
	...
	a, err := AcquirePoolSlot(context.Background(), poolReq(root, "a", "ns-a", "vea-a"))
	require.NoError(t, err)

	b, err := AcquirePoolSlot(context.Background(), poolReq(root, "b", "ns-b", "vea-b"))
	require.NoError(t, err)
	defer ReleasePoolSlot(b, "ns-b", "vea-b")

	require.NoError(t, ReleasePoolSlot(a, "ns-a", "vea-a"))

	c, err := AcquirePoolSlot(context.Background(), poolReq(root, "c", "ns-c", "vea-c"))
	require.NoError(t, err)
...
func TestAcquirePoolSlot_PreferReleasedOverHeld(t *testing.T) {
	...
	for i := uint32(0); i < PoolSize; i++ {
		l, err := AcquirePoolSlot(context.Background(), poolReq(root, fmt.Sprintf("init-%d", i), "", ""))
		require.NoError(t, err)
		held[i] = l
	}
	...
	next, err := AcquirePoolSlot(context.Background(), poolReq(root, "reuse", "", ""))
	require.NoError(t, err)
...
func TestAcquirePoolSlot_Exhaustion(t *testing.T) {
	...
	for i := uint32(1); i <= PoolSize; i++ {
		l, err := AcquirePoolSlot(context.Background(), poolReq(root, fmt.Sprintf("slot%d", i), "", ""))
		require.NoError(t, err)
		held = append(held, l)
	}

	_, err := AcquirePoolSlot(context.Background(), poolReq(root, "overflow", "", ""))
	require.Error(t, err)
...
func TestReleasePoolSlot_WritesReleasedAt(t *testing.T) {
	...
	lease, err := AcquirePoolSlot(context.Background(), poolReq(root, "release_test.bpfman:5", "ns-rel", "vea-rel"))
	require.NoError(t, err)
...
func TestAcquirePoolSlot_ConcurrentDistinctSlots(t *testing.T) {
	...
	for i := 0; i < n; i++ {
		...
		go func(i int) {
			defer wg.Done()
			results[i], errs[i] = AcquirePoolSlot(context.Background(), poolReq(root, fmt.Sprintf("goroutine-%d", i), "", ""))
		}(i)
	}
	wg.Wait()
	defer func() {
...
func TestAcquirePoolSlot_RecoversFromUnparseableBody(t *testing.T) {
	...
	lease, err := AcquirePoolSlot(context.Background(), poolReq(root, "ok", "", ""))
	require.NoError(t, err)
...
func TestAcquirePoolSlot_MkdirFailure(t *testing.T) {
	...
	_, err := AcquirePoolSlot(context.Background(), poolReq(parent+"/forbidden/pool", "x", "", ""))
	require.Error(t, err)
...
func TestPoolAcquireRequest_DefaultsApplyToEmptyFields(t *testing.T) {
	...
	lease, err := AcquirePoolSlot(context.Background(), PoolAcquireRequest{
		Root:      root,
