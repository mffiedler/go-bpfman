...
func checkSource(t *testing.T, src string) []shell.Issue {
	t.Helper()
	tokens, err := shell.Tokenise(src)
	require.NoError(t, err)
	prog, err := shell.Parse(tokens)
	require.NoError(t, err)
	return shell.Check(prog)
}
...
func TestBindShape_LinkAttachKindSpecialisesDetails(t *testing.T) {
	...
	// per-kind shape comes from reflection over the concrete
	// LinkDetails implementer registered in bindshapes.go; a
	// typo against a real TCDetails field must surface, and a
	// legitimate field must stay clean.
...
func joinIssues(issues []shell.Issue) string {
