...
// FireKind metadata and the registration API live in the repl
// package; the actual fire-worker subprocess entry points live
// in cmd/bpfman-shell (kill_helper.go, unlink_helper.go,
// uprobe_helper.go), each calling RegisterFireKind from init().
...
func RegisterFireKind(name string, k FireKind) {
	if _, dup := fireKinds[name]; dup {
		panic("repl: fire kind " + name + " registered twice")
	}
	fireKinds[name] = k
}
