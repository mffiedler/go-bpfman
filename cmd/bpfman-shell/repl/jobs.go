// Job-leak policies. Two flavours, both pure mechanism: strict
// for script and 'bpfman-shell FILE' contracts that treat a
// leaked job as a failure; silent for the interactive prompt
// and 'source' from a prompt, where starting something and
// walking away is normal exploratory use.
...
// SilentJobLeakHandler is the policy for the interactive prompt
// and for 'source' invoked from a prompt. It SIGKILLs the
// leaked process group so nothing outlives the shell, then
// returns: no diagnostic, no counter bump, no exit-code
// effect. The REPL is exploratory scratch space; starting
// something and walking away is normal use.
