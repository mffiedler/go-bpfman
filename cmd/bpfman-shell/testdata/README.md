Behavioral shell fixtures live here. They are the gold standard
...
- `testdata/<feature>/<case>/expect.yaml` holds the observable
  contract for that end-to-end behavioral fixture

Conventions:

- Use checked-in scripts for outside-in, user-visible behavior:
  script mode, imports, jobs, foreach/bind-collect, poll/retry,
  return/defer workflows, diagnostics, and cleanup.
- Prefer adding a behavioral fixture when a feature graduates from
...
- Prefer real shell behavior over synthetic placeholders:
...
- Diagnostics now carry the script file and absolute line/column
  ...
  should see.
- `expect.yaml` is required for these fixture-driven behavioral
  tests. Unknown keys are rejected so typos do not silently weaken
  coverage.
- Supported normalizers today are `jobs_listing` and
  `poll_timeout`.
