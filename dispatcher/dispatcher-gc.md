...
records for those links have captured `kernel_link_id` values that
become stale until the rebuild transaction refreshes them. Their
bpfman-managed `id` values remain stable.

If coherency logic deletes every link record whose captured
`kernel_link_id` is absent from the kernel's alive set, it will
remove every extension record after the first rebuild. The next
operation that queries extension links for the dispatcher will find
zero results, and the next rebuild will produce a dispatcher with no
extensions attached.
...
even if their captured kernel link IDs are stale.**
...
considered live regardless of their individual captured kernel link
IDs.
...
| 3     | Delete links whose captured kernel link ID is not alive | **Skips extension links of live dispatchers** |
| 4     | Delete orphaned dispatchers (zero remaining extensions) | Catches dispatchers that lost all extensions in phase 3 |

### Phase 3 detail

For each link with a captured kernel link ID:

1. **If the link is an extension (XDP or TC) whose
   `dispatcher_program_id` matches a live dispatcher, skip.** This
   is the critical guard.
2. Otherwise, if the link's kernel ID is not in the alive set,
   delete it.
...
| Store  | dispatchers: program_id=P2. E1: id=B1, kernel_link_id=K4, disp_prog_id=P2. E2: id=B2, kernel_link_id=K5, disp_prog_id=P2 |
...
| Store  | dispatchers: program_id=P3. E1: id=B1, kernel_link_id=K6, disp_prog_id=P3. E2: id=B2, kernel_link_id=K7, disp_prog_id=P3. E3: id=B3, kernel_link_id=K8, disp_prog_id=P3 |
...
| 3     | E1 (kernel_link_id=K6): is K6 alive?     | Yes. Kept            |
| 3     | E2 (kernel_link_id=K7): is K7 alive?     | Yes. Kept            |
| 3     | E3 (kernel_link_id=K8): is K8 alive?     | Yes. Kept            |
...
The store still has program_id=P3 and captured kernel link IDs K6-K8.
...
| 3     | E1 (kernel_link_id=K6): live dispatcher? | No (P3 deleted). Is K6 alive? No. **Deleted** |
| 3     | E2 (kernel_link_id=K7): same logic       | **Deleted**         |
| 3     | E3 (kernel_link_id=K8): same logic       | **Deleted**         |
...
captured kernel link IDs do not match live kernel objects. Once the
transaction commits the store is consistent again, but the next
rebuild will invalidate those captured kernel IDs once more. The
bpfman-managed link ID is the durable identity; extension kernel link
IDs are not stable across rebuilds and must not be used as durable
identity.
