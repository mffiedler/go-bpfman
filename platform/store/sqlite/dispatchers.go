...
func (s *sqliteStore) GetDispatcher(ctx context.Context, dispType dispatcher.DispatcherType, nsid uint64, ifindex uint32) (dispatcher.State, error) {
	...
	err := row.Scan(&dispTypeStr, &state.Nsid, &state.Ifindex, &state.Revision,
		&state.ProgramID, &nullLinkID, &state.Priority)
	...
	if nullLinkID.Valid {
		state.LinkID = kernel.LinkID(nullLinkID.Int64)
	}
...
func (s *sqliteStore) ListDispatchers(ctx context.Context) ([]dispatcher.State, error) {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&dispTypeStr, &state.Nsid, &state.Ifindex, &state.Revision,
			&state.ProgramID, &nullLinkID, &state.Priority); err != nil {
			s.logger.Debug("sql", "stmt", "ListDispatchers", "duration_ms", msec(time.Since(start)), "error", err)
			return nil, err
		}
		if nullLinkID.Valid {
...
func (s *sqliteStore) SaveDispatcher(ctx context.Context, state dispatcher.State) error {
	...
	// TC dispatchers have no BPF link; store link_id as NULL.
	var linkID any
	if state.LinkID != 0 {
		linkID = state.LinkID
	}
	...
	result, err := s.stmtSaveDispatcher.ExecContext(ctx,
		...
		state.ProgramID, linkID,
		state.Priority, now, now)
	if err != nil {
		s.logger.Debug("sql", "stmt", "SaveDispatcher", "args", []any{state.Type, state.Nsid, state.Ifindex, state.Revision, state.ProgramID, linkID, state.Priority, "(timestamp)", "(timestamp)"}, "duration_ms", msec(time.Since(start)), "error", err)
		return fmt.Errorf("save dispatcher: %w", err)
	}
	rows, _ := result.RowsAffected()
	s.logger.Debug("sql", "stmt", "SaveDispatcher", "args", []any{state.Type, state.Nsid, state.Ifindex, state.Revision, state.ProgramID, linkID, state.Priority, "(timestamp)", "(timestamp)"}, "duration_ms", msec(time.Since(start)), "rows_affected", rows)
