...
// source string and a Span describing the offending region, emit
...
//
// Two surface helpers: RenderDiagnostic for the full Diagnostic
// shape (with optional Help text), and RenderIssue for the common
// case where a Check Issue is the input.
...
// it points, what it says, and any inline help. Span carries the
...
// RenderIssue is sugar for RenderDiagnostic over a Check Issue.
// The Span and Msg are taken from the issue verbatim; Help is
// empty.
func RenderIssue(src, file string, issue Issue) string {
	return RenderDiagnostic(src, file, Diagnostic{
		Span: issue.Span,
		Msg:  issue.Msg,
	})
}
