...
#
# The reason this exists alongside `build-image` is convenience for
# debugging: each entry in the `contents` list is a layer added to
# the image, so adding strace, gdb, bpftrace, perf, lsof, etc. is
# literally one line here -- no Dockerfile rewrite, no apt-get,
# no rebuild of a base image. Compare to Dockerfile.bpfman.local,
...
#   build-image-multiarch  buildx multi-arch publish path used by CI;
#                          carries cosign signing and SBOM
#                          attestation, neither of which this image
#                          builder reproduces.
...
dockerTools.buildLayeredImage {
  ...
  # bpfman-operator and Dockerfile.bpfman.local both invoke the
