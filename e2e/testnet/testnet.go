...
// generated by uniqueTestName: "B", 12 hex characters, "N",
// optionally with an "a" or "b" veth suffix. Exported so
// callers (e.g. the e2e suite's TestMain) can run their own
// stale-state cleanups using the same naming convention.
var StaleTestIfaceRe = regexp.MustCompile(`^B[0-9a-f]{12}N[ab]?$`)

// CleanupStaleInterfaces removes leftover test interfaces and
// namespaces from crashed runs. Names are matched by the
// pattern generated by uniqueTestName. Idempotent; safe to call
// at TestMain entry as a pre-flight.
